    ...
    .forEach(curSelect => {
      ...
      if (findObj) {
        ...
        const parentNode = flattenData.find(
          fItem => fItem.id === findObj.parent.id
        )
        // 查找过的父元素的childNodes
...
export function optimizationTreeData (
  flattenData,
  selectData,
  selectLevel,
  targetLevel
...
function emptyListOrEmpty (data) {
  if (!data) {
    return true
  } else {
...
// 生成level map
export function getLevelMap (list) {
  let res = {}
  list.forEach(v => {
    if (emptyListOrEmpty(res[v.level])) {
      res[v.level] = []
    }
    res[v.level].push(v)
  })
