      ...
      <div class="schedule-header row">
        ...
        <div class="head-time">
          <div class="time-total row">
            <p class="border-right">上午(00:00-11:59)</p>
            <p>下午(12:00-23:59)</p>
          </div>
  ...
  methods: {
    ...
    enter(item, index) {
      ...
      this.timer = setTimeout(function() {
        let weekIndex = ~~(index / 24);
        let separatorIndex = ~~(index % 24);
    ...
    move(item, index, e) {
      ...
      if (this.moving) {
        ...
        if (this.pos.end_x >= this.pos.li_width * 47.7) {
          this.pos.end_x = this.pos.li_width * 24;
          Object.assign(this.$data.maskPos, this.$options.data().maskPos);
          this.moving = false;
    ...
    up(item, index, e) {
      if (!this.limit) {
        ...
        this.moving = false;
        // 最后一个值比第一个值大 右下
        var widthLen = (index % 24) - (this.startIndex % 24);
        // var heightLen = ~~(index / 24) - ~~(this.startIndex / 24);
    ...
    posCache(scheduleTime) {
      ...
      var startTime;
      var endTime;
      // 1.按星期 拆分数组7*24  每个单元设置  状态，开始时间，结束时间
