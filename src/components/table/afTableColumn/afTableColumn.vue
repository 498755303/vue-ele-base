<template>
  ...
  <el-table-column v-if="$scopedSlots.default"
                   v-bind="$attrs"
                   :key="$attrs.label"
  ...
  <!--使用 slot 自定义 header-->
  <el-table-column v-else-if="$scopedSlots.header"
                   v-bind="$attrs"
                   :key="$attrs.label"
                   :class-name="className"
                   :min-width="minWidth">
  ...
  <el-table-column v-else
                   ...
                   :key="$attrs.label"
                   :class-name="className"
                   :min-width="minWidth"></el-table-column>
</template>

<script>
import $consts from './constans.js';
export default {
  ...
  computed: {
    ...
    minWidth() {
      ...
      const maxOne = Math.max(this.minLength, this.$attrs.label.length * this.fontRate.CHAR_RATE) * this.fontSize + 40;
